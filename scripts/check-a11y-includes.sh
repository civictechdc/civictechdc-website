...
if grep -Eq 'h3 class="ctdc-cta-band__path-title"' _includes/components/closing-cta.html; then
  fail 'closing-cta.html must use styled <p> for path titles, not <h3>.'
fi

if grep -Eq 'h3 class="ctdc-footer__heading"' _includes/core/footer.html; then
  fail 'footer.html section headings must be <h2 class="ctdc-footer__heading">.'
fi

if ! grep -Fq 'title="Civic Tech DC upcoming events calendar"' _includes/components/meetup-events.html; then
