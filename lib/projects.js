$("#primary-content").attr("class", "col-sm-12");

var projects_table;
// Set this up like this to make it easier to dev this locally -- evz
var data_host = "https://worker.chihacknight.org";
...
$("#submit-project").click(function () {
  $.post(
    ...
    function (resp) {
      displayMessage(
        "<strong>Your project has been submitted!</strong> It should display below within 5 minutes.",
        "success",
      );
      $("#github-url").val("");
    },
    ...
    displayMessage(
      "<strong>Error submitting your project!</strong> Check that the URL is correct.",
      "danger",
    );
  });
...
<tr>\
  ...
  <td>\
    <a class='btn btn-default' href='{{html_url}}/commits/master'><i class='fa fa-check-square'></i> {{recent_commits}}</a>\
  </td>\
  <td>\
    <a class='btn btn-default' href='{{html_url}}/stargazers'><i class='fa fa-star'></i> {{watchers_count}}</a>\
  </td>\
  <td>\
    <a class='btn btn-default' href='{{html_url}}/network'><i class='fa fa-code-fork'></i> {{forks_count}}</a>\
  </td>\
  <td>\
    <a class='btn btn-default' href='{{html_url}}/issues'><i class='fa fa-exclamation'></i> {{open_issues}}</a>\
  </td>\
</tr>\
";
...
$.when($.getJSON(data_host + "/data/project_details.json")).then(
  function (data, textStatus, jqXHR) {
    ...
    $.each(data, function (i, json) {
      ...
      if (json["participation"]) {
        for (var i = 0; i < json["participation"].length; i++) {
          var val =
            ((json["participation"][i] + 1) / parseFloat(max_participation)) *
            100;
          if (val > 100) val = 100;
      ...
      else json["homepage_formatted"] = homepage;
    ...
    $("#hack-night-projects_filter input").keyup(function (e) {
      $.address.parameter(
        "search",
        $("#hack-night-projects_filter input").val(),
      );
    });
  },
);
